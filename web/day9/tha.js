...
for( let i=0;i<box.length;++i)
{
    ...
    box[i].addEventListener('click',()=>{
        ...
        if(box[i].classList.contains('boxclicked'))
        {
             ...
             count--;
             clickcount.innerHTML="<h3>Clicked box="+count+" &nbsp&nbsp&nbsp Unclicked Box="+ (36-count)+"</h3>";
        }
        else
        {
            box[i].classList.add('boxclicked');
            count++;
            clickcount.innerHTML="<h3>Clicked box="+count+" &nbsp Unclicked Box="+ (36-count)+"</h3>";
        }  
});
}